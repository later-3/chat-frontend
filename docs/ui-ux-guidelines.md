...
- 明暗均用中性底色、蓝色强调；背景、正文、弱文字、控件边界、强调按压及成功/警告/错误分别使用语义 Token。`--on-accent` 用于强调底色上的文字，不固定写白色。
...
后台工作依附当前 Friend 列表，使用既有语义色、12px 间距和轻量标题/状态列表，避免新增第二个主导航。创建表单折叠，工作名称与完整说明分别输入；提交未确认时保留重试入口，不伪装成功。选中的工作明确显示固定项目与“返回日常交流”，切换全局项目不能悄悄改变工作。桌面操作最小 32px，紧凑触摸场景 44px，输入字号 16px；状态不用仅颜色表达。打开工作复用中央会话和过程/结尾区。
