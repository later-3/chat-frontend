...
function hasOnlyDeviceKeys(value: Record<string, unknown>): boolean {
  return Object.keys(value).every((key) => DEVICE_KEYS.has(key));
}
...
function validateDevice(value: unknown, label: string): DeviceValidationResult {
  ...
  if (!hasOnlyDeviceKeys(value)) {
    return { diagnostic: diagnostic("invalid-device", `${label} contains unsupported fields`) };
  }
 ...
 * Invalid remote entries never make the current Chat instance unusable.
 */
