...
test("New restores the draft after session navigation and workspace auto-restore", async (t) => {
  ...
  for (const rememberedCwd of ["/draft-project", "/draft-project-worktree"]) {
    await t.test(`remembered session cwd: ${rememberedCwd}`, async () => {
      ...
      const session = { id: "remembered", cwd: rememberedCwd, projectKey: cwd, owner: { type: "ordinary" } };
      ...
      const context = vm.createContext({
        ...
        router: { replace() {}, push() {} },
        projectDraftKey: (cwd) => JSON.stringify(["test-device", "new", cwd]),
        projectIdsByCwd: { current: new Map() },
        setNavigationError() {},
        activateChat() {},
        parseSessionListPage: (value) => value,
        fetch: () => response.promise,
        deviceId: "test-device",
        getLastOpenSession: (key) => key === JSON.stringify(["test-device", cwd]) ? session.id : null,
        clearLastOpen() {},
        workspaceKeyOf: (value) => value.projectKey ?? value.cwd,
        useCallback: (callback) => callback,
        useGlobalKeyboardShortcuts() {},
        activeNewSessionDraftKeyRef: { current: JSON.stringify(["test-device", "new", cwd]) },
        ...
        contentPanel: "sessions",
        lastCoworkerRef: { current: null },
        isMobile: false,
        activeCwd: cwd,
      ...
      async function commit() {
        if (mountedKey !== context.sessionKey) {
          ...
          const key = context.selectedSession ? null : context.projectDraftKey(activeCwd);
