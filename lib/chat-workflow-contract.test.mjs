...
test("prompt input supports new and existing sessions", () => {
  ...
  // The session-memory switch travels with the send; absent means the default (the memory node runs).
  assert.deepEqual(
    parseChatWorkflowPromptInput({
      projectId: "chat", cwd: "/repo", prompt: "这轮别写记忆", workflow: "minimal-pi-coding-agent", sessionMemory: "off",
    }),
    { projectId: "chat", cwd: "/repo", prompt: "这轮别写记忆", workflow: "minimal-pi-coding-agent", sessionMemory: "off" },
  );
  assert.deepEqual(
