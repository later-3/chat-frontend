...
test("Long Agent settings renders separated source-owned tabs and guarded Memory actions", async () => {
  ...
  assert.match(settings, /type SettingsTab = "runtime" \| "tasks" \| "duties" \| "deliverables" \| "activity" \| "conversations" \| "topics" \| "agent-group" \| "agent-memory"/);
  assert.match(settings, /LongAgentTopicsPanel/);
  assert.match(settings, /LongAgentConversationsPanel/);
  assert.match(settings, /LongAgentDutiesSettings/);
  assert.match(settings, /LongAgentDeliverablesSettings/);
