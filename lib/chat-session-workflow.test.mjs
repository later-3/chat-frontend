...
test("clarification reviews expose blocking questions without an approval action", () => {
  assert.match(chatWindowSource, /review\.readiness === "needs_clarification"/);
  assert.match(chatWindowSource, /review\.blockingQuestions\.map/);
  assert.match(chatWindowSource, /\{!needsClarification && \(/);
  assert.match(chatWindowSource, /chat\.submitPlanClarification/);
});
