...
test("Moments uses a global main page and keeps the chat mounted", async () => {
  ...
  assert.match(app, /data-workspace-chat hidden=\{workspaceView !== "chat"\}/);
  assert.match(app, /enabled: workspaceView === "chat"/);
  assert.ok(sidebar.indexOf('className={modeStyles.globalNavigation}') < sidebar.indexOf('{/* CWD picker */}'));
