...
const summary = {
  ...
  members: [{
    ...
    hasParticipationSession: true, sessionId: "01a0c962-0614-7a61-bdab-1e70c35b413d", grants: { systemToolAddresses: [], nativeTools: ["read"], extensionTools: [] },
  }],
...
test("conversation summary rejects malformed or widened shapes", () => {
  ...
  // The owner-facing participation Session id: parsed when present, null when absent or explicit null.
  assert.equal(parseConversationSummary(summary).members[0].sessionId, "01a0c962-0614-7a61-bdab-1e70c35b413d");
  assert.equal(parseConversationSummary(summary).publicSessionId, "s1");
  assert.throws(() => parseConversationSummary({ ...summary, publicSessionId: "" }));
  assert.equal(parseConversationSummary({ ...summary, members: [{ ...summary.members[0], sessionId: null }] }).members[0].sessionId, null);
  assert.equal(parseConversationSummary({ ...summary, members: [{ ...summary.members[0], sessionId: undefined }] }).members[0].sessionId, null, "a legacy backend without the field stays compatible");
  assert.throws(() => parseConversationSummary({ ...summary, members: [{ ...summary.members[0], sessionId: 17 }] }));
