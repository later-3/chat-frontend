...
test("Project列表保留Daily类型并拒绝未知类型", async (t) => {
  const previousFetch = globalThis.fetch;
  t.after(() => { globalThis.fetch = previousFetch; });
  const daily = {
    projectId: "daily",
    cachedName: "Daily",
    cachedDescription: "默认日常Project",
    path: "/chat-home/workspaces/daily",
  ...
  globalThis.fetch = async () => new Response(JSON.stringify({ projects: [project, daily] }), {
    status: 200,
    headers: { "Content-Type": "application/json" },
  });
  assert.deepEqual(await fetchChatProjects(), [daily, project]);

  globalThis.fetch = async () => new Response(JSON.stringify({
    projects: [{ ...daily, kind: "system" }],
  }), {
    status: 200,
    headers: { "Content-Type": "application/json" },
  });
