...
test("Project sidebar and execution route use Session owner as their only Long Agent identity", async () => {
  ...
  // 选中长期同事会话不改变顶部上下文项目（AppShell 用 owner.type 分流）。
  ...
  assert.match(sidebar, /registeredProjects\.find\(\(project\) => project\.kind === "share"\)/);
  assert.match(appShell, /contextCwd \?\? selectedSession\.cwd/);
  assert.match(appShell, /if \(session\.owner\?\.type !== "long-agent"\) setContextCwd\(session\.cwd\)/);
