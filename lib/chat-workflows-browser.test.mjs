...
test("Agent resolve responses are validated through every nested resource boundary", async (t) => {
  ...
  assert.equal(parsed.agent.modelSource, "chat-default");
  assert.equal(parsed.agent.thinkingSource, null);

  globalThis.fetch = async () => new Response(JSON.stringify({
    ...response,
    agent: { ...response.agent, modelSource: "from-elsewhere" },
...
test("fetchChatModelCatalog validates the Backend model catalog", async (t) => {
  ...
  assert.equal(catalog.thinkingLevels.length, 7);
  ...
  globalThis.fetch = async () => new Response(JSON.stringify({
    schemaVersion: 1, providers: [], models: [], thinkingLevels: "high",
  }), {
    status: 200,
    headers: { "Content-Type": "application/json" },
  });
  await assert.rejects(fetchChatModelCatalog(), /\u6a21\u578b\u76ee\u5f55/);
});
