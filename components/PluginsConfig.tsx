...
export function PluginsConfig({
  ...
  const loadPlugins = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/plugins?projectId=${encodeURIComponent(projectId)}&cwd=${encodeURIComponent(cwd)}`);
  ...
  const runAction = useCallback(async (action: PluginAction, pkg: PluginPackageInfo) => {
    ...
    try {
      const res = await fetch("/api/plugins", {
        ...
        body: JSON.stringify({ projectId, action, source: pkg.source, scope: pkg.scope, cwd }),
      });
      const next = (await res.json()) as PluginsResponse & { error?: string };
      if (!res.ok || next.error) throw new Error(next.error ?? `HTTP ${res.status}`);
      setData(next);
  ...
  const installPlugin = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/plugins", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ projectId, action: "install", source, scope: installScope, cwd }),
      });
      const next = (await res.json()) as PluginsResponse & { error?: string };
      if (!res.ok || next.error) throw new Error(next.error ?? `HTTP ${res.status}`);
      setData(next);
  ...
  }, [cwd, installScope, installSource, projectId]);
