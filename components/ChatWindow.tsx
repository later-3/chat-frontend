...
function phaseLabel(phase: AgentPhase, t: (key: string, params?: Record<string, string | number>) => string): string | null {
  ...
  if (phase?.kind === "workflow_stage") {
    if (phase.stage.nodeKind === "task") return t("chat.waitingPlanReview");
    return phase.stage.agentId === "planner"
      ? t("chat.plannerWorking")
      : t("chat.agentWorking", { name: phase.stage.agentId === "pi-coding-agent" ? "Pi Coding Agent" : (phase.stage.agentId ?? phase.stage.stageId) });
  }
...
export function ChatWindow({ projectId, session, sessionRunning, newSessionCwd, newSessionDraftKey, onAgentEnd, onAttentionNeeded, onSessionCreated, onSessionOpen, onSessionForked, chatInputRef, onBranchDataChange, onSystemPromptChange, onSystemPromptLoaderChange, onSessionStatsChange, onSessionStatsPanelOpen, onContextUsageChange, onOpenFile, onConnectionFailure, soundEnabled = true, onSoundToggle, playDoneSound = () => {}, unlockAudio }: Props) {
  ...
  const {
    ...
    agentPhase, activeRunStage, planReview, reviewSubmitting,
    isNew,
      ...
      <div className="relative flex min-w-0 flex-1 overflow-hidden">
        <div ref={scrollContainerRef} className="min-w-0 flex-1 overflow-x-hidden overflow-y-auto pt-4 [scrollbar-width:none]">
          <div style={{ minWidth: 0, padding: `0 ${CHAT_COLUMN_PADDING}px` }}>
                ...
                if (processCount > 0) {
                  ...
                  const processGroup = (
                    <ProcessDetailsGroup
                      ...
                      defaultExpanded={
                        !finalAnswerMessage
                        || visibleProcessIndices.some((processIdx) => (
                          messages[processIdx]?.role === "assistant"
                          && (messages[processIdx] as AssistantMessage).chatWorkflow?.agentId === "planner"
                        ))
                      }
                      t={t}
            ...
            {agentRunning && !planReview && !hasStreamingContent && agentPhase && (
