...
export const MessageView = memo(function MessageView({ message, isStreaming, toolResults, cwd, onOpenFile, entryId, onFork, forking, onNavigate, prevAssistantEntryId, onEditContent, showTimestamp, prevTimestamp, sessionId, writtenFiles }: Props) {
  ...
  if (message.role === "custom") {
    ...
    if ((message as CustomMessage).customType === "chat.plan_review_decision") {
      return <PlanReviewDecisionMessageView message={message as CustomMessage} />;
    }
    ...
    >
      {delegatedBy && (
        <div style={{
          ...
          <span style={{ color: "var(--accent)", fontWeight: 600 }}>{t("chat.delegatedTask")}</span>
          <span aria-hidden="true">·</span>
          <span>{t("chat.delegatedByWorkflowAgent", {
            workflow: delegatedBy.workflowId,
            agent: delegatedBy.agentId,
          })}</span>
        </div>
      )}
...
/**
 * Legacy plan approvals have no native User Message on disk. Keep their
 * internal role honest while presenting the human action like a conversation
...
function PlanReviewDecisionMessageView({ message }: { message: CustomMessage }) {
  ...
  return (
    <div
      aria-label={t("chat.humanReview")}
      style={{ marginBottom: 16, display: "flex", flexDirection: "column", alignItems: "flex-end" }}
    >
      <div style={{ width: "fit-content", maxWidth: "85%" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 5, margin: "0 4px 4px", color: "var(--text-muted)", fontSize: 11, fontWeight: 600 }}>
