...
function PiWebTitle() {
  ...
  const target = showVersion ? `${import.meta.env.VITE_APP_VERSION ?? "0.1.1"}p${import.meta.env.VITE_PI_VERSION ?? "source"}` : "Chat";
...
export function SessionSidebar({ contentPanel, onContentPanelChange: setContentPanel, projectSlot, filesSlot, momentsActive = false, onOpenMoments, selectedSession, selectedSessionId, newSessionDraftKey, onSelectSession, onOpenSessionById, onNewSession, initialSessionId, initialSessionProjectId, skipInitialProjectSelection, onInitialRestoreDone, onReady, refreshKey, onSessionRemoved, selectedCwd: selectedCwdProp, selectedProjectId, onCwdChange, onOpenFile, explorerRefreshKey, onExplorerRefresh, onAtMention, onAtMentions, mobileView = "sessions", onMobileViewChange, onRequestClose, onBackgroundTaskDone, onRunningSessionIdsChange }: Props) {
  ...
  const [removedSessionsOpen, setRemovedSessionsOpen] = useState(false);
  ...
  const hasSessionActivity = runningSessionIds.size > 0
    || allSessions.some((session) => session.attention !== undefined);
  useEffect(() => {
    let cancelled = false;
    ...
    const schedule = () => {
      ...
      timer = setTimeout(async () => {
        ...
        schedule();
      }, hasSessionActivity ? 1_000 : 4_000);
    };
    const handleVisibilityChange = () => {
      if (timer !== undefined) clearTimeout(timer);
      timer = undefined;
      if (document.visibilityState === "visible") {
        void loadSessions(false, true).finally(schedule);
      }
    };
    schedule();
  ...
  }, [hasSessionActivity, loadSessions]);
  ...
  }, [validatedProject, worktreeState, allSessions, registeredProjects, projectSelection, selectedCwdProp, selectedProjectId]);
  ...
  const commitCustomPath = useCallback(async (candidate?: string) => {
    ...
    try {
      const data = await openChatProject(path);
      setValidatedProject({
        cwd: data.cwd,
  ...
  const projectActivity = useMemo(
    () => getProjectActivity(displayedSessions, runningSessionIds, unreadSessionIds),
    [displayedSessions, runningSessionIds, unreadSessionIds],
  );
  ...
  const sessionTree = buildSidebarSessionTree(visibleSessions);
          ...
          >
              ...
              <div style={{ maxHeight: "min(50vh, 380px)", overflowY: "auto" }}>
                {visibleProjects.map((project) => (
                  <button
                    key={project.key}
                    onClick={() => void commitCustomPath(project.root)}
                    style={{
                      display: "flex",
                      alignItems: "center",
                      gap: 7,
...
function SessionTreeItem({
  ...
  const descendantAttentionCount = node.attentionCount - (node.session.attention === undefined ? 0 : 1);
    ...
    <div>
      <div style={{ position: "relative" }}>
        ...
        <SessionItem
          ...
          descendantAttentionCount={descendantAttentionCount}
          onClick={() => onSelectSession(node.session)}
          onRenamed={onRenamed}
          onRemoved={(id) => onSessionRemoved?.(id)}
          depth={depth}
...
function AttentionSessionIndicator({
  attention,
  descendantCount,
}: {
  ...
  const { t } = useI18n();
  const label = attention?.kind === "clarification"
    ? t("sidebar.needsClarification")
    : attention?.kind === "review"
      ? t("sidebar.needsReview")
      : t("sidebar.childNeedsAttention", { count: descendantCount });
  return (
    <span
      title={label}
      aria-label={label}
...
function SessionItem({
  ...
  const diskActionsDisabled = readOnly || session.transient === true || isRunning === true;
  ...
  const commitRename = useCallback(async () => {
    ...
    setActionError(null);
    try {
      if (!session.projectId) throw new Error("Session缺少Project ID，无法重命名");
      await renameSession(session.projectId, session.id, name);
      onRenamed?.();
    } catch (error) {
      setActionError(error instanceof Error ? error.message : String(error));
    }
  }, [renameValue, session.id, session.name, session.projectId, onRenamed, title]);
  ...
  const performRemove = useCallback(async () => {
    ...
    setActionError(null);
    try {
      if (!session.projectId) throw new Error("Session缺少Project ID，无法移除");
      await removeSession(session.projectId, session.id);
  ...
  }, [diskActionsDisabled, performRemove]);

  const handleRemoveConfirm = useCallback((e: React.MouseEvent) => {
    e.stopPropagation();
    void performRemove();
  }, [performRemove]);

  const handleRemoveCancel = useCallback((e: React.MouseEvent) => {
    e.stopPropagation();
    setConfirmRemove(false);
  }, []);
  ...
  return (
    <div
      ...
      style={{
        ...
        paddingRight: 8,
        cursor: confirmRemove || renaming ? "default" : "pointer",
    ...
    >
      {confirmRemove ? (
        /* ── Remove confirmation: same height, two flat buttons ── */
        <>
          ...
          <div style={{ display: "flex", gap: 5, flexShrink: 0 }}>
            ...
            >
              <IconArchive size={12} stroke={2} aria-hidden="true" />
              {t("sidebar.delete")}
            </button>
            <button
        ...
        /* ── Normal view ── */
        <>
          {/* Parent/child Session indicator (Pi fork lineage or Workflow call). */}
          {depth > 0 && (
            ...
            <div style={{ marginTop: 2, display: "flex", alignItems: "center", gap: 8, color: "var(--text-dim)", fontSize: 11, minWidth: 0 }}>
              {actionError ? (
                ...
                <AttentionSessionIndicator
                  ...
                  descendantCount={descendantAttentionCount}
                />
              ) : isRunning ? (
            ...
            <>
              ...
              <div className="mobile-action-sheet" role="dialog" aria-modal="true" aria-label={`Session actions for ${title}`}>
                ...
                <div className="mobile-action-grid">
                  ...
                  >
                    <IconArchive size={21} stroke={1.7} aria-hidden="true" />
