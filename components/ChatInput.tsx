...
export const ChatInput = forwardRef<ChatInputHandle, Props>(function ChatInput({
  ...
  useEffect(() => {
    ...
    void fetchChatWorkflowSummaries(controller.signal)
      .then(setWorkflowSummaries)
      .catch((error: unknown) => {
        if (!(error instanceof DOMException && error.name === "AbortError")) {
          console.error("Failed to load Chat Workflows", error);
        }
      });
    return () => controller.abort();
  }, []);

  const visibleWorkflows = workflowSummaries.some((workflow) => workflow.id === workflowId)
    ? workflowSummaries
    : [
        {
          id: workflowId,
          name: workflowId === DEFAULT_CHAT_WORKFLOW_ID ? t("chat.workflowDirect") : workflowId,
          description: "",
          agentCallable: false,
          planReview: false,
  ...
  return (
    ...
    >
      ...
      {workflowAgentDialogOpen && cwd && selectedWorkflow && selectedWorkflow.agents.length > 0 && (
        <WorkflowAgentConfigDialog
          workflow={selectedWorkflow}
          projectId={projectId}
          cwd={cwd}
          configs={workflowAgentConfigs}
          proposals={promptResourceProposals ?? []}
          onConfigsChange={onWorkflowAgentConfigsChange}
          onClose={() => setWorkflowAgentDialogOpen(false)}
        />
      )}
    </div>
  );
