...
globalThis.self = {
  location: {
    href: "https://chat.test/sw.js?v=test",
...
test("notification click focuses an existing client at the session URL", async () => {
  const calls = [];
  const focusedClient = {
    url: "https://chat.test/?session=session-1",
...
test("notification click navigates an existing client to the session", async () => {
  ...
  const existingClient = {
    url: "https://chat.test/?session=other-session",
    navigate: async (url) => {
  ...
  assert.deepEqual(calls, [
    ["navigate", "https://chat.test/?session=session-1"],
    "focus",
  ]);
});
...
test("notification click opens a window and rejects cross-origin targets", async () => {
  ...
  assert.deepEqual(opened, ["https://chat.test/"]);
});
